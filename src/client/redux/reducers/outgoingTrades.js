import { types } from '../actions/outgoingTrades'
...
export default (state = initialState, action) => {
  ...
  if (action.type === types.ADD) {
    ...
    return { ...state, response: [...trades, trade ], count, isLoading: false }
  }
  if (action.type === types.REMOVE) {
    const id = action.payload
    const count = state.count--
    const trades = state.response ? state.response : []
    return { ...state, response: trades.filter(trade => trade._id != id), count, isLoading: false }
  }
  return state
}